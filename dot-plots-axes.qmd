...
Thresh = E1_c@model$Theta
Slopes = E1_c@model$coefficients

cols <- brewer.pal(brewer.pal.info["RdYlGn", "maxcolors"], "RdYlGn")[1:7]

emmeans(E1_c@model, ~ condition) %>% 
  as_tibble() %>%
  mutate_at(vars("emmean":"asymp.UCL"), as.numeric) %>%
  ggplot(aes(x = emmean, y = condition, group = 1)) +
  ...
  scale_y_discrete(labels = c("", "Low", "High"),
                   limits = c("", "lo", "hi")) +
  lims(x = c(min(Thresh)-0.5,
             max(Thresh)+0.5)) +
  geom_vline(xintercept = c(unname(Thresh))) +
  theme_minimal(base_size = 18) +
  ...
  annotate("text", 
           ...
           size = 3) +
  annotate("rect", 
           xmin = c(-Inf, unname(Thresh)), 
           xmax = c(unname(Thresh), Inf),
           ymin = -Inf, ymax = Inf, 
           fill = cols,
           alpha = 0.3)
...
The odds ratio for the difference between conditions is `r printnum(1/exp(E2_c.estimate))`, `r print_confint(1/exp(E2_c.CI))`. Participants were `r round(1/exp(E2_c.estimate), 1)` times more likely to award a higher magnitude rating to data points presented at *low* positions than data points presented at high positions. This is equivalent to a Cohen's d value of `r printnum(oddsratio_to_d(1/exp(E2_c.estimate)))`. 
...
Thresh = E2_c@model$Theta
Slopes = E2_c@model$coefficients

cols <- brewer.pal(brewer.pal.info["RdYlGn", "maxcolors"], "RdYlGn")[1:7]

emmeans(E2_c@model, ~ condition) %>% 
  as_tibble() %>%
  mutate_at(vars("emmean":"asymp.UCL"), as.numeric) %>%
  ggplot(aes(x = emmean, y = condition, group = 1)) +
  geom_linerange(aes(xmin = asymp.LCL, xmax = asymp.UCL),
...
The odds ratio for the difference between conditions is `r printnum(1/exp(E2_s.estimate))`, `r print_confint(1/exp(E2_s.CI))`. Participants were `r round(1/exp(E2_s.estimate), 1)` times more likely to award a higher severity rating to data points presented at low positions than data points presented at high positions. This is equivalent to a Cohen's d value of `r printnum(oddsratio_to_d(1/exp(E2_s.estimate)))`. 
