...
Experiment 2 used the same y-axis manipulation as Experiment 1, but in charts with inverted y-axis orientations, where data points at lower physical positions represented greater values. This allowed us to investigate whether magnitude judgements were driven by data points' absolute positions, or their relative positions within the context of the axis limits. Importantly, the use of inverted charts should not be considered an endorsement (see issues above). However, they serve to distinguish between two possible explanations, since they reverse the typical associations between physical position and magnitude.
...
#| fig-alt: "Horizontal stacked bar charts, showing the responses at each Likert scale category for data points presented at high and low physical positions, shown separately in Experiment 1 (conventional axis orientations) and Experiment 2 (inverted axis orientations). The title reads 'Ratings of Data Points' Magnitudes'. The label 'Very unlikely' appears on the far left, and the label 'Very likely' appears on the far right. In Experiment 1, for the 'high' condition, there are a greater proportion of responses at the 'Very likely' end of the scale. For the 'low' condition, there are a greater proportion of responses at the 'Very unlikely' end of the scale. In Experiment 2, this pattern is reversed."
...
combined_plot <- likert_plot(combined_df) + facet_wrap(~ experiment, ncol = 1, labeller = labeller(experiment = ori_labs)) +
  labs(title = "Ratings of Data Points' Magnitudes")
...
@fig-combined plots the distribution of ratings of data points' magnitudes. Values presented at high physical positions elicited a greater proportion of responses at the higher end of the rating scale than values presented at low physical positions.
...
@fig-combined plots the distribution of ratings of data points' magnitudes. Values presented at *low* physical positions elicited a greater proportion of responses at the higher end of the rating scale than values presented at *high* physical positions.
