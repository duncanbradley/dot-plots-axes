...
The odds ratio for the difference between conditions is `r printnum(exp(E1_c@model$beta))`. Participants were `r round(exp(E1_c@model$beta), 1)` times more likely to award a higher magnitude rating to data points presented at high positions than low positions. This is equivalent to a Cohen's d value of `r printnum(oddsratio_to_d(exp(E1_c@model$beta)))`. 
...
The odds ratio for the difference between conditions is `r printnum(exp(E1_s@model$beta))`. Participants were `r round(exp(E1_s@model$beta), 1)` times more likely to award a higher severity rating to data points presented at high positions than low positions. This is equivalent to a Cohen's d value of `r printnum(oddsratio_to_d(exp(E1_s@model$beta)))`.
...
The odds ratio for the difference between conditions is `r printnum(exp(E2_c@model$beta))`. Participants were `r round(exp(E2_c@model$beta), 1)` times more likely to award a higher magnitude rating to data points presented at high positions than low positions. This is equivalent to a Cohen's d value of `r oddsratio_to_d(exp(E2_c@model$beta))`. 
...
The odds ratio for the difference between conditions is `r printnum(exp(E2_s@model$beta))`. Participants were `r round(exp(E2_s@model$beta), 1)` times more likely to award a higher severity rating to data points presented at high positions than low positions. This is equivalent to a Cohen's d value of `r printnum(oddsratio_to_d(exp(E2_s@model$beta)))`.
...
We also generated two additional models, to test whether or not the above results could be explained by differences in graph literacy. These models were identical to the above models except for the inclusion of participants' graph literacy scores as an additional fixed effect. Adjusting for participants' graph literacy scores did not change the pattern of results regarding ratings of the magnitude of data points themselves (z = `r printnum(abs(E2_cl.statistic))`, p `r printp(E2_cl.p.value, add_equals = TRUE)`, odds ratio = `r printnum(exp(E2_cl$beta))`) or ratings of the severity of consequences (z = `r printnum(abs(E2_sl.statistic))`, p `r printp(E2_sl.p.value, add_equals = TRUE)`, odds ratio = `r printnum(exp(E2_cl$beta))`).
