...
For magnitude ratings, there was an interaction between data visualisation literacy and physical position: z = `r printnum(abs(E2_clint.cl.statistic))`, p `r printp(E2_clint.cl.p.value, add_equals = TRUE)`, odds ratio = `r printnum(exp(E2_clint.cl.estimate))`, `r print_confint(exp(E2_clint.cl.CI))`. This is equivalent to a Cohen's d value of `r printnum(abs(oddsratio_to_d(exp(E2_clint.cl.estimate))))`, a `r paste(interpret_cohens_d(oddsratio_to_d(exp(E2_clint.cl.estimate))))` effect size. The difference between magnitude ratings for data points presented at different positions diminished as data visualisation literacy increased. This model employed random intercepts for scenarios with slopes for literacy.
